            ...
            for sfn, swt, sv in iter_fields(v):
                if sfn == 1 and swt == 0:
                    ...
                    for tfn, twt, tv in iter_fields(sv):
                        # tfn=4 is the active station, 0-INDEXED (verified by a controlled
                        # start-sweep: Valve1->0, Valve2->1, Valve3->2, Valve4->3), so the
                        # 1-indexed UI valve = tv + 1. tfn=1 is a constant run-type (2), NOT
                        # the station — reading it lit every valve as #2.
                        if tfn == 4 and twt == 0:
                            st.active_zone = tv + 1
